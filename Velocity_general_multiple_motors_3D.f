	...
	open(unit=21,file="Rate_V_clus_motran_D1_N10.dat")
        open(unit=23,file="V_clus_motran_D1_N10.dat")
c        open(unit=20, file="V_confare_motran_D001_N5.dat")
c        open(unit=22, file="V_avgarea_motran_D001_N5.dat")
c        open(unit=24, file="V_clus_rate_motran_D001_N5.dat")
     ...
     1		(beadmotz(i)-beadinitz)**2)
	   ...
	   if(term_diff .gt. 1e-4)then
		go to 1
          ...
          b_dis= sqrt((beadmoty(i)**2)+(beadmotz(i)**2))
          if(b_dis .gt. Lo)go to 1
c          b_dis= 1000*sqrt((beadmoty(i)**2)+(beadmotz(i)**2))
c          if(b_dis .gt. 6.125)go to 1
